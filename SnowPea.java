public class SnowPea extends Plant implements Runnable{
	public SnowPea(int xPos, int yPos, Stage stage){
		super(xPos,yPos,96,99,"SnowPea.png",stage,TOUGHNESS_NORMAL);//int xPos, int yPos,int width,int height, String filename,Stage stage,int hp
		this.actionSpd=1500;
	}

	public void shoot(){ //create peas until dead
		if(stage.zombieCheck(this.xPos,this.yPos)&&!this.suspendFlag) 
			stage.addParticle(new FrozenPea(this.xPos,this.yPos+22,stage));
	}
